...
def play_game():
    ...
    def spawn_zombie():
        ...
        zombie_y = random.randint(WINDOW_HEIGHT + 50, WINDOW_HEIGHT + 70)  # Zombies spawn below the screen
        speed_up = random.uniform(0.09, 0.12)  # Vitesse de montée plus lente
        speed_down = random.uniform(0.18, 0.24)  # Vitesse de descente plus rapide
    ...
    while waiting_input:
        ...
        # Mouvement des zombies (avec différentes vitesses de montée et descente)
        for zombie in zombie_list:
            if zombie["direction"] == "up":
                zombie["y"] -= zombie["speed_up"]  # Zombie monte avec la vitesse de montée
                if zombie["y"] < WINDOW_HEIGHT // 4:  # Point où le zombie commence à redescendre
                    zombie["direction"] = "down"  # Zombie commence à redescendre
            elif zombie["direction"] == "down":
                zombie["y"] += zombie["speed_down"]  # Zombie descend avec la vitesse de descente
                if zombie["y"] > WINDOW_HEIGHT + 50:  # Zombie atteint le bas de l'écran
                    zombie["direction"] = "up"  # Zombie recommence à monter
                    zombie["y"] = random.randint(WINDOW_HEIGHT + 50, WINDOW_HEIGHT + 150)  # Réinitialise sa position en bas
